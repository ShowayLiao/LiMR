# modified from https://github.com/zhangzilongc/LiMR
...
class MVTecDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.source = source
        self.split = split
        # 如果classname不为空, 则只使用classname, 否则使用_CLASSNAMES（MVtec数据集中的所有类别）
        ...
        # for train
        self.transform_img_LiMR = transforms.Compose([
            transforms.RandomResizedCrop(imagesize,
                                         scale=(cfg.TRAIN.LiMR.DA_low_limit,
                                                cfg.TRAIN.LiMR.DA_up_limit),
        ...
        transform_mask = [
            transforms.Resize((resize, resize)),
            transforms.CenterCrop(imagesize),# 围绕中心裁剪到符合imagesize的尺寸
            transforms.ToTensor(),
        ]
    ...
    def __getitem__(self, idx):
        ...
        if self.split.value == "train":
            image = self.transform_img_LiMR(image)
        else:
