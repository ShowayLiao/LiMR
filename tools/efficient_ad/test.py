#!/usr/bin/env python3
"""EfficientAD 测试脚本：使用训练好的 EfficientAD 模型进行数据集评估。
...
用法示例:
  # MVTec-AD 单类别测试
  python tools/efficient_ad/test.py --dataset mvtec --root ./datasets/MVTec --category bottle \
      --checkpoint ./output_efficient_ad/EfficientAD/MVTecAD/bottle/v1/weights/lightning/model.ckpt

  # VisA 测试
  python tools/efficient_ad/test.py --dataset visa --root ./datasets/VisA --category capsules \
      --checkpoint ./output_efficient_ad/EfficientAD/Visa/capsules/v1/weights/lightning/model.ckpt
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=f"EfficientAD 测试脚本。支持数据集: {', '.join(DATASET_REGISTRY)}",
    ...
    # ---- 输出 ----
    parser.add_argument("--output-dir", type=str, default="./output_efficient_ad_test")
...
def main():
    ...
    print("=" * 80)
    print(f"EfficientAD 测试 - 数据集: {args.dataset}")
    print("=" * 80)
